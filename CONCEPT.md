...
**ARGUS-OS1 — открытая платформа для непрерывного 3D-трекинга центриолей в живых клетках.** Измеряет траекторию каждой центриоли и судьбу клетки. Данные передаются в ARGUS-OS2.

> **Почему это возможно именно на ARGUS:** непрерывный режим 24/7 (нет пропусков между сессиями) + AI-трекинг (CellPose 2.0 + Bayesian tracker) позволяют отслеживать отдельные центриоли через 2-3 клеточных цикла. Стандартная микроскопия с перерывами теряет треки. ARGUS — не теряет.
