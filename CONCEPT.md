...
| 3. Mitosis + spindle | SiR-Tubulin (640 nm) | Chromatin condensation trigger. **Record XY spindle vector** (Z negligible in monolayer). |
...
> **Минимальный набор маркеров:** Centrin1-GFP (позиция центриолей) + SiR-Tubulin (вектор веретена, **XY** — достаточно для монослоя RPE1. Z-компонент веретена в плоских клетках <2 µm — пренебрежим). + Arl13B (цилия, endpoint). **Возраст = количество пройденных делений.** Судьба = Arl13B⁺.
